...
function GlRenderer(canvas, maxVertexCnt, isImg, imgPath, videoElement) {
    ...
    this.isImg = isImg;
    this.callback = videoElement;

    this.init();

    this.updateImage(imgPath, this.callback);
...
GlRenderer.prototype.renderTriangles = function(iw, ih) {
    ...
    for(var i = triangles.length - 1; i > 2; i -= 3) {
        ...
        var pa = [vertices[triangles[i]][0] * this.canvas.width,
                vertices[triangles[i]][1] * this.canvas.height];
        var pb = [vertices[triangles[i - 1]][0] * this.canvas.width,
                vertices[triangles[i - 1]][1] * this.canvas.height];
        var pc = [vertices[triangles[i - 2]][0] * this.canvas.width,
                vertices[triangles[i - 2]][1] * this.canvas.height];
        ...
        PolyPoints.push({"point": [pa, pb, pc], "color": rgb})
    }
